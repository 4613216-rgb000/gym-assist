...
export default function Home() {
  ...
  const toIsoFromCandidate = (cand?: string) => {
    ...
    const normalize = (s: string) => {
      ...
      const dtag = day ?? (x.toLowerCase().startsWith('tomorrow') ? 'tomorrow' : 'today');
      if (hh === undefined) {
      ...
      if (dtag === 'dayafter') {
        ...
        const d = now.getDate() + 2;
        const local = new Date(y, mo, d, hh, mi ?? 0, se ?? 0, 0);
        return local.toISOString().replace('.000Z', 'Z');
      }
      const hhStr = String(hh).padStart(2, '0');
      const miStr = String(mi ?? 0).padStart(2, '0');
      const seStr = String(se ?? 0).padStart(2, '0');
      return `${dtag} ${hhStr}:${miStr}:${seStr}`;
    };
    const c2 = normalize(cand) ?? cand;
    const m = c2.match(/^(today|tomorrow)\s+(\d{1,2}):(\d{2})(?::(\d{2}))?$/);
    if (!m) return typeof c2 === 'string' && /Z$/.test(c2) ? c2 : undefined;
    const now = new Date();
    const dayOffset = m[1] === 'tomorrow' ? 1 : 0;
    const y = now.getFullYear();
    const mo = now.getMonth();
    const d = now.getDate() + dayOffset;
    const hh2 = Number(m[2]);
    const mi2 = Number(m[3]);
    const se2 = m[4] ? Number(m[4]) : 0;
    const local = new Date(y, mo, d, hh2, mi2, se2, 0);
    return local.toISOString().replace('.000Z', 'Z');
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const llmIsoForPreview = toIsoFromCandidate(data.entities.time) || toIsoFromCandidate((data.entities.timeCandidates || [])[0]) || toIsoFromCandidate(input);
      setLlmPreviewIso(llmIsoForPreview);
      if (data.intent === 'createTodo') {
        const computedISO = toIsoFromCandidate(data.entities.time) || toIsoFromCandidate((data.entities.timeCandidates || [])[0]) || toIsoFromCandidate(input);
        ...
        const todoRes = await fetch(`${base}/api/v1/todos`.replace('//api', '/api'), {
          ...
          body: JSON.stringify({
            title: data.entities.title,
            description: data.entities.description,
            dueTime: toIsoFromCandidate(data.entities.time) || toIsoFromCandidate((data.entities.timeCandidates || [])[0]) || toIsoFromCandidate(input),
            priority: data.entities.priority || 1,
            tags: data.entities.tags || [],
          }),
        });
        ...
        const trigIso = toIsoFromCandidate(data.entities.time) || toIsoFromCandidate((data.entities.timeCandidates || [])[0]) || toIsoFromCandidate(input);
        ...
        const rw = await fetch(`${base}/api/v1/llm/rewrite?mode=light`.replace('//api', '/api'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}`, ...(apiKey ? { 'X-API-Key': apiKey } : {}) },
          body: JSON.stringify({ title: data.entities.title, description: data.entities.description, dueTime: data.entities.time, timezoneOffsetMinutes: new Date().getTimezoneOffset() })
        });
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(async () => {
      ...
      try {
        ...
        const data = await res.json();
        const iso = toIsoFromCandidate(data.entities?.time) || toIsoFromCandidate((data.entities?.timeCandidates || [])[0]) || toIsoFromCandidate(input);
              ...
              <ul className="space-y-2">
                {today.map((t) => (
                  <li key={t.id} className="bg-gray-200 rounded px-3 py-2 flex items-center justify-between">
                    ...
                    <div className="text-sm text-gray-700">{t.due_time ? (() => { const d = new Date(t.due_time); const hh = String(d.getHours()).padStart(2, '0'); const mm = String(d.getMinutes()).padStart(2, '0'); const ss = String(d.getSeconds()).padStart(2, '0'); return `${hh}:${mm}:${ss}` })() : 'time'}</div>
                  </li>
                ))}
              </ul>
            )}
            ...
            {weekWindow.map((t) => (
              <li key={t.id} className="border rounded p-3 flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-600">{t.due_time ? new Date(t.due_time).toLocaleDateString() : (lang === 'zh' ? '日期' : 'date')}</div>
                </div>
